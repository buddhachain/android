 ...
 * 更新善举
 */
public class UpdateShanjvActivity extends BaseActivity {
        ...
        XuperApi.kinddeedDetail(kdid, new ResponseCallBack<String>() {
            ...
            public void onSuccess(String resp) {
                try {
                    ...
                    String[] list = resp.split("\\{");
                    mImageList.clear();
                    mImageList.addAll(Arrays.asList(list));
                    mImageList.remove(0);
                    mAddImageAdapter.notifyDataSetChanged();
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    void submit() {
        ...
        DialogUtil.simpleDialog(mContext, "确认上传？", new DialogUtil.ConfirmCallBackObject<String>() {
            @Override
            public void onConfirmClick(String content) {
                XuperApi.updateKinddeed(kdid, name, mSelectShanjvType, detail, spec, new ResponseCallBack<String>() {
                    @Override
                    public void onSuccess(String resp) {
